...
if "(\<exists>p'. spectroscopy_moves (Attacker_Delayed p Q) (Attacker_Delayed p' Q) = (Some id)
\<and> in_wina e (Attacker_Delayed p' Q)
\<and> strategy_formula_conjunction (Attacker_Delayed p' Q) e \<chi>)"|

observation: 
"strategy_formula_conjunction (Attacker_Delayed p Q) e (Obs \<alpha> \<phi>)" 
if "\<exists>p' Q'. spectroscopy_moves (Attacker_Delayed p Q) (Attacker_Immediate p' Q') = (subtract 1 0 0 0 0 0 0 0)
...
if "(if Q = {} then (\<exists>p'. spectroscopy_moves (Attacker_Immediate p Q) (Defender_Conj p' Q') = (Some id)
\<and> in_wina e (Defender_Conj p' Q')
\<and> strategy_formula (Defender_Conj p' Q') e \<phi>)
else \<exists>p'. spectroscopy_moves (Attacker_Immediate p Q) (Defender_Conj p' Q') = (subtract 0 0 0 0 1 0 0 0)
\<and> in_wina (e - (E 0 0 0 0 1 0 0 0)) (Defender_Conj p' Q')
\<and> strategy_formula (Defender_Conj p' Q') (e - (E 0 0 0 0 1 0 0 0)) \<phi>)"|

late_conj:
"strategy_formula_conjunction (Attacker_Delayed p Q) e \<chi>"
if "(spectroscopy_moves (Attacker_Delayed p Q) (Defender_Conj p Q) = (Some id)
\<and> (in_wina e (Defender_Conj p Q))
...
if "(\<exists>Q'. spectroscopy_moves (Attacker_Clause p q) (Attacker_Delayed p Q') = Some min1_6
\<and> in_wina (min1_6 e) (Attacker_Delayed p Q')
...
if "(\<exists>P'. spectroscopy_moves (Attacker_Clause p q) (Attacker_Delayed q P') = Some (min1_7 \<circ> (\<lambda>x. x- E 0 0 0 0 0 0 0 1))
...
if "spectroscopy_moves (Attacker_Delayed p Q) (Defender_Stable_Conj p Q) = (Some id)
\<and> in_wina e (Defender_Stable_Conj p Q) 
\<and> strategy_formula (Defender_Stable_Conj p Q) e \<chi>"|

branch:
"strategy_formula (Attacker_Delayed p Q) e \<chi>" 
if "\<exists>p' Q' \<alpha> Q\<alpha>. spectroscopy_moves (Attacker_Delayed p Q) (Defender_Branch p' \<alpha> p'' Q' Q\<alpha>) = (Some id)
\<and> in_wina e (Defender_Branch p' \<alpha> p'' Q' Q\<alpha>)
