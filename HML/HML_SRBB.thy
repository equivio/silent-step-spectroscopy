...
"(\<forall>i \<in> I. (\<exists>\<phi>. ((F i)) = (HML_not (HML_internal \<phi>)) \<or> ((F i)) = HML_just (HML_internal \<phi>) \<and> is_hml_chi (Inl \<phi>)))" |
(*\<And>{(\<alpha>)\<phi>, \<psi>, \<psi>, ...} where HML_soft_poss \<alpha> \<phi> stands for (\<alpha>)\<phi>*)
chi_conj_bb: "is_hml_chi (Inl (HML_conj I F))" if
"((\<exists>i \<in> I. \<exists>\<alpha> \<phi>. (F i) = (HML_soft_poss \<alpha> \<phi>) \<and> is_hml_chi (Inl \<phi>)
    ...
    \<and> (\<forall>j \<in> I. i \<noteq> j \<longrightarrow> 
      (\<exists>\<phi>. ((F j)) = (HML_not (HML_internal \<phi>)) \<or> ((F j)) = HML_just (HML_internal \<phi>) \<and> is_hml_chi (Inl \<phi>)))))" |
(*\<And>{\<not>\<langle>\<tau>\<rangle>T, \<psi>, \<psi>, ...}*)
chi_conj_sr: "is_hml_chi (Inl (HML_conj I F))" if
