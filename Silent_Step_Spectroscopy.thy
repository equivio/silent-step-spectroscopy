...
theorem spectroscopy_game_correctness:
  ...
  shows \<open>(\<exists>\<phi>. distinguishes_from \<phi> p Q \<and> expressiveness_price \<phi> \<le> e)
       \<longleftrightarrow> weak_spectroscopy_game.attacker_wins e (Attacker_Immediate p Q)\<close>
proof
  ...
  thus \<open>weak_spectroscopy_game.attacker_wins e (Attacker_Immediate p Q)\<close>
    using weak_spectroscopy_game.win_a_upwards_closure \<phi>_spec(2) by simp
next
  assume \<open>weak_spectroscopy_game.attacker_wins e (Attacker_Immediate p Q)\<close>
...
proposition attacker_subet_wins:
  assumes
    \<open>weak_spectroscopy_game.attacker_wins e (Attacker_Immediate p Q)\<close>
