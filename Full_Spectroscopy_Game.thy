...
fun spectroscopy_moves :: "('s, 'a) position \<Rightarrow> ('s, 'a) position \<Rightarrow> energy update option" where 
  delay: 
    "spectroscopy_moves (attacker_basic p Q) (attacker_delay p' Q') 
      = (if p' = p \<and> Q \<Zsurj>S Q' then Some id else None)" |

  procrastination: 
    "spectroscopy_moves (attacker_delay p Q) (attacker_delay p' Q')     
      = (if (Q' = Q \<and> p \<noteq> p' \<and> p \<mapsto> \<tau> p') then Some id else None)" |

  observation: 
    "spectroscopy_moves (attacker_delay p Q) (attacker_basic p' Q') 
  ...
  finishing_or_early_conj: 
    "spectroscopy_moves (attacker_basic p Q) (defender_conj p' Q') 
      = (if Q = {} \<and> Q' = {} then 
  ...
  late_inst_conj: 
    "spectroscopy_moves (attacker_delay p Q) (defender_conj p' Q') 
  ...
  pos_neg_clause: 
    "spectroscopy_moves (attacker_clause p q) (attacker_delay p' Q') 
      = (if (p = p') then 
  ...
  late_stbl_conj: 
    "spectroscopy_moves (attacker_delay p Q) (defender_s_conj p' Q') 
      = (if (p = p' \<and> Q' = { q \<in> Q. (\<nexists>q'. q \<mapsto>\<tau> q')} \<and> (\<nexists>p''. p \<mapsto>\<tau> p'')) then Some id else None)" |

  conj_s_awns: 
    "spectroscopy_moves (defender_s_conj p Q) (attacker_clause p' q) 
      = (if p = p' \<and> q \<in> Q then (subtract 0 0 0 1 0 0 0 0) else None)" |

  br_conj: 
    "spectroscopy_moves (attacker_delay p Q) (defender_branch p' \<alpha> p'' Q' Q\<alpha>) 
