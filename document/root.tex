...
\paragraph{Overview.}
By phrasing \emph{behavioral equivalences as energy games}, one obtain a uniform way to decide a wide range of equivalences in van Glabbeek's \emph{linear-time--branching-time spectrum} for systems with \emph{silent steps}~\cite{glabbeek1990ltbt1,glabbeek1993ltbt2}.
Silent $\tau$-steps usually arise because of abstraction from internal system behavior, for instance, when modeling communication protocols or distributed systems using transition systems.
...
\begin{itemize}
  ...
  \item Sections 4 to 6 prove certain coordinates to correspond to weak trace equivalence, $\eta$-bi\-si\-mi\-la\-ri\-ty, $\eta$-similarity and branching bisimilarity as well as their stable variants. (As the relationship is established through modal logics, the results can me understood as Hennessy--Milner theorems~\cite{hm1980hml}.)
...
\noindent
The bigger project of \emph{deciding all equivalences at once} is outlined in Bisping's PhD thesis~\cite{bisping2025generalizedEqChecking}.The energy game approach is due to~\cite{bisping2023equivalenceEnergyGames}.

\paragraph{Acknowledgements.}
Several proofs in this document are due to pen-and-paper proofs by David N. Jansen.
