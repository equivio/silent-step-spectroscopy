...
If \<open>e\<close> is in the winning budget of \<open>Attacker_Immediate p Q\<close>, there is a strategy formula \<open>\<phi>\<close> 

HOW DO WE PRONOUNCE \<open>\<phi> \<in> Strat((p,Q), e)\<close>?

with expressiveness price \<open>\<le> e\<close>. 

We prove a more detailed result for all possible game positions \<open>g\<close> by induction.
While the paper uses an order over pairs of game positions and energies to do a well-founded 
induction we use the induction rules given by the definition of winning budgets:
...
Since we only look at direct successors we added the not too interesting case of \<open>g\<close> being an 
attacker branching position which was omitted in the paper.

LINK to formalization changes
\<close>
